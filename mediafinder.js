...
var GLOBAL_config = {
  ...
  HEADERS: {
    ...
    'Accept-Language':
      'en-US,en;q=0.8,fr-FR;q=0.6,fr;q=0.4,de;q=0.2,de-DE;' +
...
var mediaFinder = {
  search: function search(service, query, userAgent, callback) {
    GLOBAL_config.HEADERS['User-Agent'] =
      userAgent ||
    ...
    function cleanMicropost(micropost) {
      ...
      if (micropost) {
        ...
        cleanedMicropost = cleanedMicropost.replace(
          GLOBAL_config.URL_REGEX,
          ' '
        );
        // simplify #hashtags to hashtags
        cleanedMicropost = cleanedMicropost.replace(
          GLOBAL_config.HASHTAG_REGEX,
          ' $2'
        );
        // simplify @username to username
        cleanedMicropost = cleanedMicropost.replace(
          GLOBAL_config.USER_REGEX,
          ' $2'
        );
        // simplify +username to username
        cleanedMicropost = cleanedMicropost.replace(
          GLOBAL_config.PLUS_REGEX,
          ' $2'
        );
        ...
        jsdom.env(body, function (errors, window) {
          ...
          try {
            if ($.getElementsByTagName('VIDEO').length > 0) {
              mediaUrl = body.substring(
                body.indexOf('<source src="') + '<source src="'.length
              );
        ...
        */
        options = {
          headers: {
            ...
            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
            Authorization:
              'GoogleLogin auth=' + GLOBAL_config.GOOGLE_RESEARCH_API_KEY,
          },
          method: 'POST',
    ...
    function collectResults(json, service, pendingRequests) {
      if (GLOBAL_config.DEBUG) {
        console.log('collectResults for ' + service);
      }
      /* global io:false */
    ...
    var services = {
                ...
                if (
                  ...
                  item.object.attachments.forEach(function (attachment) {
                    ...
                    var micropost = cleanMicropost(
                      (item.object.content ? item.object.content : '') +
      ...
      Facebook: function (pendingRequests) {
        ...
        var params = {
          ...
          limit: 100,
          fields:
            'comments,type,created_time,name,caption,description,' +
            'source,picture,id,from,likes,shares',
          access_token:
            GLOBAL_config.FACEBOOK_APP_ID +
            '|' +
              ...
              Step(
                function () {
                  ...
                  items.forEach(function (item) {
                    ...
                    var mediaUrl =
                      item.type === 'video' ? item.source : item.picture;
                    if (item.type === 'video') {
                      cleanVideoUrl(mediaUrl, function (cleanedMediaUrl) {
                        if (
                          cleanedMediaUrl &&
                          ...
                          results.push({
                            mediaUrl: cleanedMediaUrl.replace(
                              /s\.jpg$/gi,
                              'n.jpg'
                            ),
      ...
      Instagram: function (pendingRequests) {
        ...
        var options = {
          url:
            ...
            query
              .replace(/\s*/g, '')
              .replace(GLOBAL_config.INSTAGRAM_REGEX, '')
              .toLowerCase() +
            '/media/recent?' +
            params,
          headers: GLOBAL_config.HEADERS,
          ...
          try {
            ...
            if (body.data && body.data.length) {
              ...
              for (var i = 0, len = items.length; i < len; i++) {
                ...
                var micropost = '';
                micropost +=
                  item.caption && item.caption.text ? item.caption.text : '';
                micropost +=
                  (micropost.length ? '. ' : '') +
                ...
                results.push({
                  ...
                  micropost: cleanMicropost(micropost),
                  userProfileUrl:
                    'https://api.instagram.com/v1/users/' + item.user.id,
                  ...
                  for (var i = 0, len = items.length; i < len; i++) {
                    ...
                    request.get(options, function (err, reply, body2) {
                      try {
                        ...
                        var micropost =
                          body2.post.title + '. ' + item.post.description;
            ...
            if (body && body.query && body.query.pages) {
              for (var pageId in body.query.pages) {
                ...
                results.push({
                  ...
                  micropost: cleanMicropost(
                    title.replace(/(\.jpg|\.png|\.gif|\.webp)$/gi, '')
                  ),
                  userProfileUrl:
